...
app.post('/api/auth/login', (req, res) => {
  ...
  // Query database for user with matching name and password
  const query = 'SELECT id, name, email, role, status FROM users WHERE name = ? AND password = ? AND status = "active"';
  ...
  db.query(query, [name, password], (err, results) => {
    ...
    if (results.length === 0) {
      console.log('Login failed: Invalid credentials for user:', name);
...
// Verify user session/token (for protected routes)
app.get('/api/auth/verify', (req, res) => {
  // This would be used with JWT tokens, for now just return success
